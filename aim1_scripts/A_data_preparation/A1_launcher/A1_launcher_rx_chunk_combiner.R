...
date_of_rx_chunk_input <- "20250620"
...
jid <- SUBMIT_ARRAY_JOB(
  ...
  memory = "250G", #determine based off running a few individual runner scripts and checking the Rstudio environment to see how much memory it uses
  threads = 1, # can usually keep at 1 unless the code itself is doing something complicated
  time = "72:00:00", # determine based off running a runner scripts and timing the entire script to get an estimate, then add +15 min
