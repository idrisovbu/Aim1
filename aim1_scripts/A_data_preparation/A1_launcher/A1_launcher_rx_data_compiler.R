...
log_dir <- file.path("/mnt/share/limited_use/LU_CMS/DEX/hivsud/aim1/A_data_preparation/", date_folder)
...
jid <- SUBMIT_ARRAY_JOB(
  ...
  memory = "250G", #determined by running on May 18th
  threads = 1, # can usually keep at 1 unless the code itself is doing something complicated
  time = "16:00:00", # determine based off running a runner scripts and timing the entire script to get an estimate, then add +15 min
