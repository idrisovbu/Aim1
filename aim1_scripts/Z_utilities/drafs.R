...
if (interactive()) {
  path <- "/mnt/share/limited_use/LU_CMS/DEX/hivsud/aim1/A_data_preparation/20250518/aggregated_by_year/compiled_F2T_data_2019.parquet"
...
##----------------------------------------------------------------
## Keep relevant variables and limit to analytic sub population (WHT, BLCK, HISP)
##----------------------------------------------------------------
#df <- df[race_cd %in% c("WHT", "BLCK", "HISP")]
...
#   has_hiv = levels(df$has_hiv), # has_hiv
...
for (b in seq_len(B)) {
  ...
  # Ensure has_hiv has both levels so pivot_wider works correctly
  tmp <- tmp %>%
    mutate(has_hiv = factor(has_hiv, levels = c(0, 1)))
