...
path <- "/mnt/share/limited_use/LU_CMS/DEX/01_pipeline/MDCR/run_77/F2T/data/toc=IP/year_id=2016/code_system=icd10/age_group_years_start=85/familyfam_neo-0.parquet"
...
), by = .(bene_id, acause, race_cd, sex_id)]
...
if (interactive()) {
  ...
  df <- read_parquet(path) %>% sample_n(10000000)
  df <- as.data.table(df)  
  year_id <- 2010
  file_type <- "F2T"
} else {
...
# Removing rows where acause_lvl2 is "hiv" to avoid tautological interaction with has_hiv flag.
# This allows meaningful interpretation of has_hiv × acause_lvl2 interactions (e.g., HIV's impact on CVD).
# We retain STD-related causes under the same acause_lvl2_lvl2 group ("HIV/AIDS and sexually transmitted infections"),
...
df_bins_master <- df %>%
  group_by(acause_lvl2, race_cd, sex_id, age_group_years_start, toc) %>%
...
grid_input_master <- df_bins_master %>%
  ...
  left_join(df_bins_master, by = c("acause_lvl2", "race_cd", "sex_id", "age_group_years_start", "toc"))

# crossing(has_hiv = factor(c(0, 1), levels = levels(df$has_hiv))) %>%
# left_join(df_bins_master, by = c("acause_lvl2", "race_cd", "sex_id", "age_group_years_start", "toc", "code_system"))
...
for (b in seq_len(B)) {
  ...
  # logistic regression
  mod_logit <- glm(
    has_cost ~ acause_lvl2 * has_hiv + acause_lvl2 * has_sud + race_cd + sex_id,
    #has_cost ~ acause_lvl2*has_hiv + race_cd + sex_id,
    data=df_boot,
  ...
  mod_gamma <- glm(
    tot_pay_amt ~ acause_lvl2 * has_hiv + acause_lvl2 * has_sud + race_cd + sex_id,
    #tot_pay_amt ~ acause_lvl2*has_hiv + race_cd + sex_id,
    data = df_gamma_input,
  ...
  # Summarise bootstrap iteration
  # out_b <- tmp %>%
  #   group_by(acause_lvl2, race_cd, has_hiv, age_group_years_start, toc, code_system) %>%
  #   summarise(
  #     cost_est = sum(exp_cost * prop_bin, na.rm = TRUE),
  #     .groups = "drop"
  #   ) %>%
  #   pivot_wider(
  #     id_cols = c(acause_lvl2, race_cd, age_group_years_start, toc, code_system),
  #     names_from = has_hiv,
  ...
  #   )
  # out_b <- tmp %>%
  #   group_by(acause_lvl2, race_cd, has_hiv, has_sud, age_group_years_start, toc, code_system) %>%
  #   summarise(
  #     cost_est = sum(exp_cost * prop_bin, na.rm = TRUE),
  #     .groups = "drop"
  #   ) %>%
  ...
  #   )
  out_b <- tmp %>%
    group_by(acause_lvl2, race_cd, has_hiv, has_sud, age_group_years_start, toc) %>%
    summarise(cost_est = sum(exp_cost * prop_bin, na.rm = TRUE), .groups = "drop") %>%
    pivot_wider(
      id_cols = c(acause_lvl2, race_cd, age_group_years_start, toc),
      names_from = c(has_hiv, has_sud),
      values_from = cost_est,
      names_prefix = "cost_hiv",
      names_sep = "_"
...
df_summary <- boot_combined %>%
  ...
  left_join(df_bins_summary, by = c("acause_lvl2", "race_cd", "age_group_years_start", "toc"))
