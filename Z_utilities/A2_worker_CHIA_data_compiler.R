##----------------------------------------------------------------
##' Title: 02_worker_data_complier_chia.R
##' Purpose:
##'
##' ----------------------------------------------------------------
rm(list = ls())
...
chia_df <- read_parquet('/mnt/share/limited_use/LU_CMS/DEX/01_pipeline/CHIA_MDCR/run_77/CAUSEMAP/data/carrier=false/toc=AM/year_id=2015/st_resi=MA/age_group_years_start=65/sex_id=1/hop_part0.parquet')
chia_df_carrier_true <- read_parquet('/mnt/share/limited_use/LU_CMS/DEX/01_pipeline/CHIA_MDCR/run_77/CAUSEMAP/data/carrier=true/toc=AM/year_id=2019/st_resi=MA/age_group_years_start=65/sex_id=1/carrier_part0.parquet')
####THIS below is likley what needed given presence of all needed variables. 
chia_df_primary_cause <- read_parquet('/mnt/share/limited_use/LU_CMS/DEX/01_pipeline/CHIA_MDCR/run_77/PRIMARY_CAUSE/data/toc=AM/year_id=2015/age_group_years_start=70/sex_id=1/part0.parquet')

colnames(chia_df_primary_cause)


if (interactive()) {
  # Set filepath for parameters_aims1.csv, this has all filepaths for parquet files
  fp_parameters <- paste0(l, "LU_CMS/DEX/hivsud/aim1/resources_chia/parameters_chia.csv")
...
dir.create(chia_chunk_folder, showWarnings = FALSE, recursive = TRUE)
...
for (i in 1:nrow(data_dirs)) {
  ...
  row <- data_dirs[i, ]
  dir <- row$directory
  chia_chunk_folder <- file.path(output_folder, "chia_chunks",
  ...
  chunk_filename <- sprintf("chia_%s_age%s_sex%s_state%s.parquet",
                            row$year_id,
  ...
  # Full output path
  chunk_path <- file.path(chia_chunk_folder, chunk_filename)
