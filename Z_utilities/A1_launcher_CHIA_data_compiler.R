...
run_id <- 77
fp_input_data <- paste0("/mnt/share/limited_use/LU_CMS/DEX/01_pipeline/CHIA_MDCR/run_", run_id, "/CAUSEMAP/data/carrier=false")
...
# Define file path
fp_parameters <- paste0(l, "/LU_CMS/DEX/hivsud/aim1/resources_chia/")

# Ensure the directory exists before writing the file
#if (!dir.exists(fp_parameters)) dir.create(dirname(fp_parameters), recursive = TRUE, showWarnings = FALSE)
if (!dir.exists(fp_parameters)) dir.create(fp_parameters, recursive = TRUE, showWarnings = FALSE)


# Write full directory paths to CSV
fp_parameters_full_directories <- paste0(fp_parameters, "parameters_chia.csv")
...
# Write to CSV
fp_parameters_unique <- paste0(fp_parameters, "parameters_chia_unique.csv")
